...
public class BlurEffect {
    ...
    public static Framebuffer renderBlur1(float step) {
        Shaders.renderFullImageInFBO(BLUR_H2, Shaders.BLUR,
                uniformCache -> uniformCache.glUniform2F("blurDir", 0, step)).bindFramebufferTexture();
        Shaders.renderFullImageInFBO(BLUR_W2, Shaders.BLUR,
                uniformCache -> uniformCache.glUniform2F("blurDir", step, 0)).bindFramebufferTexture();
        Shaders.renderFullImageInFBO(BLUR_H, Shaders.BLUR, uniformCache -> uniformCache.glUniform2F("blurDir", 0, step))
                .bindFramebufferTexture();
        Shaders.renderFullImageInFBO(BLUR_W, Shaders.BLUR, uniformCache -> uniformCache.glUniform2F("blurDir", step, 0))
                .bindFramebufferTexture();
        return BLUR_W;
    }
